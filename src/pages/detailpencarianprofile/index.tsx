...
import { ButtonC, DataKomentar, Header, ImageBg, InputKomentar, Like, ModalC, Pin, Select, ViewAddKomentar, container } from "../../components";
                    ...
                    <View marginT-10 style={{ flexDirection: 'row', justifyContent: 'center' }}>
                        ...
                        <ButtonC
                            ...
                            onPress={async () => {
                                ...
                                await setLoad(true)
                                await deleteFoto({ id: route.params.id }).then((res) => {
                                    if (res.IsError === false) {
                                        setLoad(false)
                                        navigation.navigate('TabProfile', { id: route.params.id })
                                    } else {
                                    ...
                                    if (err.response) {
                                        console.error('Error hapus Foto Response Data:', err.response.data);
                                        console.error('Error hapus Foto Response Status:', err.response.status);
                                    } else if (err.request) {
                                        console.error('Error hapus Foto Request:', err.request);
                                    } else {
                                        console.error('Error hapus Foto Message:', err.message);
                                    }
